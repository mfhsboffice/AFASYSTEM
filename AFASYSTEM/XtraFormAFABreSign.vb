...
Public Class XtraFormAFABreSign
...
#Region "Form"
    ...
    Private Sub XtraFormAFABreSign_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        SetupEditors()
        SetupGrid()
        LoadApproverLists()
        LoadPriority()
...
#Region "Setup"
    ...
    Private Sub SetupEditors()
        For Each editor As TextEdit In New TextEdit() {TextEditBudgetAmtSource, TextEditBudgetAmtTarget,
                                                       ...
                                                       TextEditEstimationSource, TextEditShortageSource,
                                                       TextEditReclassAmount, TextEditBalanceTarget,
        ...
        TextEditBudgetItemSource.Properties.ReadOnly = True
        TextEditBudgetItemTarget.Properties.ReadOnly = True
        TextEditScheduleFrom.Properties.ReadOnly = True
        TextEditScheduleTo.Properties.ReadOnly = True
...
#Region "Load"
    ...
    Private Sub LoadAll(ByVal afaNo As String)
        ...
        Try
            ...
            If Convert.ToString(row("AFA_TYPE")) <> "BRE" Then
                XtraMessageBox.Show("AFA " & afaNo & " is not a Reclass Budget document.",
                                    "Signature AFA Reclass Budget",
                                    MessageBoxButtons.OK, MessageBoxIcon.Warning)
                ClearForm()
                Return
            ...
            TextEditScheduleFrom.Text = FormatDate(row("AFA_PER_FROM"))
            TextEditScheduleTo.Text = FormatDate(row("AFA_PER_TO"))
            ...
            Me.Text = "Signature AFA Reclass Budget - " & _afaNo &
                      " (" & _headerStatus & ")" &
    ...
    Private Sub LoadFigures()
        Dim dt As DataTable = _signature.GetReclassFigures(_afaNo)
        ...
        Dim target As DataRow = Nothing

        For Each row As DataRow In dt.Rows
            If Convert.ToString(row("ITEM_ROLE")) = "Source" Then source = row
            If Convert.ToString(row("ITEM_ROLE")) = "Target" Then target = row
        Next

        If source IsNot Nothing Then
            TextEditBudgetItemSource.Text = Convert.ToString(source("BUDGET_ITEM_CODE"))
            TextEditBudgetAmtSource.Text = FormatAmount(source("BUDGET_AMOUNT"))
            TextEditActualUpSource.Text = FormatAmount(source("ACTUAL_UP"))
            TextEditEstimationSource.Text = FormatAmount(source("ESTIMATION"))
            TextEditShortageSource.Text = FormatAmount(source("SHORTAGE"))
        End If

        If target IsNot Nothing Then
            TextEditBudgetItemTarget.Text = Convert.ToString(target("BUDGET_ITEM_CODE"))
            TextEditBudgetAmtTarget.Text = FormatAmount(target("BUDGET_AMOUNT"))
            TextEditActualUpTarget.Text = FormatAmount(target("ACTUAL_UP"))
            TextEditReclassAmount.Text = FormatAmount(target("RECLASS_AMOUNT"))
            TextEditBalanceTarget.Text = FormatAmount(target("BALANCE"))
        End If

        TextEditTotalReclass.Text = If(target Is Nothing, "0", FormatAmount(target("RECLASS_AMOUNT")))
    End Sub

    Private Sub ClearFigures()
        TextEditBudgetItemSource.Text = ""
        TextEditBudgetItemTarget.Text = ""
    ...
    Private Sub PickAttachment(ByRef target As String, ByVal editor As DevExpress.XtraEditors.ButtonEdit)
        If _afaNo = "" Then
            XtraMessageBox.Show("Please load a document first.", "Signature AFA Reclass Budget",
                                MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Return
        End If
        ...
        If _headerStatus <> "Draft" Then
            XtraMessageBox.Show("Attachments can only be added while the document is a Draft.",
                                "Signature AFA Reclass Budget", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Return
        End If
...
#Region "Save"
    ...
    Private Sub BtnSave_Click(sender As Object, e As EventArgs) Handles BtnSave.Click
        ...
        If _dtNodes Is Nothing OrElse _dtNodes.Rows.Count = 0 Then
            XtraMessageBox.Show("There are no signature nodes to save.", "Signature AFA Reclass Budget",
                                MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Return
        End If
        ...
        Try
            ...
            XtraMessageBox.Show("Signature saved.", "Signature AFA Reclass Budget",
    ...
    Private Sub BtnSend_Click(sender As Object, e As EventArgs) Handles BtnSend.Click
        ...
        Try
            If _signature.Submit(_afaNo, _nik, _pc) Then
                XtraMessageBox.Show(_signature.LastErrorMessage, "Signature AFA Reclass Budget",
                                    MessageBoxButtons.OK, MessageBoxIcon.Information)
                LoadAll(_afaNo)
            Else
...
#Region "Events"
    ...
    Private Sub BtnViewAFA_Click(sender As Object, e As EventArgs) Handles BtnViewAFA.Click
        ...
        XtraMessageBox.Show("The document view is not available yet.", "Signature AFA Reclass Budget",
                            MessageBoxButtons.OK, MessageBoxIcon.Information)
    End Sub
...
#Region "Helpers"
    ...
    Private Sub ClearForm()
        ...
        ClearFigures()
        TextEditScheduleFrom.Text = ""
        TextEditScheduleTo.Text = ""
        SelectPriority.SelectedIndex = 0
        GridControlSignature.DataSource = Nothing
