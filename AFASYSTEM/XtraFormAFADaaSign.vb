...
Public Class XtraFormAFADaaSign
...
#Region "Form"
    ...
    Private Sub XtraFormAFADaaSign_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        SetupEditors()
        SetupGrid()
        LoadApproverLists()
        LoadPriority()
...
#Region "Setup"
    ...
    Private Sub SetupEditors()
        ...
        DateEditScheduleFrom.Properties.ReadOnly = True
        DateEditScheduleTo.Properties.ReadOnly = True
...
#Region "Load"
    ...
    Private Sub LoadAll(ByVal afaNo As String)
        ...
        Try
            ...
            If Convert.ToString(row("AFA_TYPE")) <> "DAA" Then
                XtraMessageBox.Show("AFA " & afaNo & " is not a Disposal document.",
                                    "Signature AFA Disposal",
                                    MessageBoxButtons.OK, MessageBoxIcon.Warning)
                ClearForm()
                Return
            ...
            DateEditScheduleFrom.EditValue = If(IsDBNull(row("AFA_PER_FROM")), Nothing, row("AFA_PER_FROM"))
            DateEditScheduleTo.EditValue = If(IsDBNull(row("AFA_PER_TO")), Nothing, row("AFA_PER_TO"))
            ...
            Me.Text = "Signature AFA Disposal - " & _afaNo &
                      " (" & _headerStatus & ")" &
    ...
    Private Sub LoadFigures()
        Dim dt As DataTable = _signature.GetDisposalFigures(_afaNo)
        ...
        TextEditAcquisition.Text = FormatAmount(row("ACQUISITION"))
        TextEditAccumDep.Text = FormatAmount(row("ACCUM_DEPRECIATION"))
        TextEditBookValue.Text = FormatAmount(row("BOOK_VALUE"))
        TextEditResellValue.Text = FormatAmount(row("RESELL_VALUE"))
        TextEditProfitLoss.Text = FormatAmount(row("PROFIT_LOSS"))
    End Sub

    Private Sub ClearFigures()
        TextEditAcquisition.Text = "0"
        TextEditAccumDep.Text = "0"
        TextEditBookValue.Text = "0"
        TextEditResellValue.Text = "0"
        TextEditProfitLoss.Text = "0"
    End Sub
...
#Region "Save"
    Private Sub BtnSave_Click(sender As Object, e As EventArgs) Handles BtnSave.Click
        ...
        If _dtNodes Is Nothing OrElse _dtNodes.Rows.Count = 0 Then
            XtraMessageBox.Show("There are no signature nodes to save.", "Signature AFA Disposal",
                                MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Return
        End If
        ...
        Try
            ...
            XtraMessageBox.Show("Signature saved.", "Signature AFA Disposal",
    ...
    Private Sub BtnSend_Click(sender As Object, e As EventArgs) Handles BtnSend.Click
        ...
        Try
            If _signature.Submit(_afaNo, _nik, _pc) Then
                XtraMessageBox.Show(_signature.LastErrorMessage, "Signature AFA Disposal",
                                    MessageBoxButtons.OK, MessageBoxIcon.Information)
                LoadAll(_afaNo)
            Else
...
#Region "Helpers"
    ...
    Private Sub ClearForm()
        ...
        SetButtonState(False)
        Me.Text = "Signature AFA Disposal"
    End Sub
